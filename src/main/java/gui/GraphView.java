    ...
    class GraphPanel extends JComponent {
        ...
        public void update(int step, game.Map map, MapStats stats) {
            if (classes.size() >= 2) {
                ...
                stats.initialize(map);
                stats.generateCount(map);
                int count1 = stats.getNbPig();
                int count2 = stats.getNbHuman();
                int count3 = stats.getNbDucks();
                int count4 = stats.getNbChicken();
                ...
                countLabel.setText(stats.getLivingDetails(map));
            }
        }
